...
String format(DateTime date,
  ...
  final String locale = injector.get<AppPresenter>().language;
  ...
  final LookupMessages messages = _lookupMessagesMap[locale] ?? EnMessages();
  final DateTime timeClock = clock ?? DateTime.now();
  int elapsed = timeClock.millisecondsSinceEpoch - date.millisecondsSinceEpoch;

  String prefix, suffix;

  if (allowFromNow && elapsed < 0) {
    elapsed = date.isBefore(timeClock) ? elapsed : elapsed.abs();
