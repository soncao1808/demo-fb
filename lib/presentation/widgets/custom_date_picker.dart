        ...
        if (showDayOfWeek) {
          for (int wd = 1; wd < DateTime.daysPerWeek; wd++) {
            final String dayOfMonth =
                localizations.datePickerDayOfMonth(longestDayOfMonth);
            if (longestText.length < dayOfMonth.length) {
              longestText = dayOfMonth;
            }
          }
...
class _CupertinoTimerPickerState extends State<CupertinoTimerPicker> {
  ...
  void initState() {
    super.initState();
    selectedAmPm = widget.initialTimerDuration.inHours ~/ 12;
    ...
    hourController = FixedExtentScrollController(
        initialItem: widget.initialTimerDuration.inHours);

    minuteController = FixedExtentScrollController(
        initialItem:
            widget.initialTimerDuration.inMinutes ~/ widget.minuteInterval);
  ...
  void _pickerDidStopScrolling() {
    ...
    _scrollToDate(widget.minimumTime);
    _scrollToDateMinute(widget.minimumTime);
  }

  void _scrollToDate(Duration? minimumTime) {
    SchedulerBinding.instance.addPostFrameCallback((Duration timestamp) {
      if (minimumTime != null &&
          selectedHour != null &&
          (selectedHour! < minimumTime.inHours)) {
        _animateColumnControllerToItem(hourController, minimumTime.inHours);
      }
    });
  }
        ...
        children: List<Widget>.generate(24, (int index) {
          ...
          Widget renderBody() {
            if (widget.minimumTime != null &&
                ...
                (index < widget.minimumTime!.inHours)) {
              return Container(
                width: _kTimerPickerColumnIntrinsicWidth +
                    additionalPadding.horizontal,
                padding: additionalPadding.resolve(textDirection),
                alignment:
  ...
  // One or more pickers have just stopped scrolling.
  void _pickerDidStopScrollingMinute() {
    setState(() {});
  ...
  void _scrollToDateMinute(Duration? minimumTime) {
    SchedulerBinding.instance.addPostFrameCallback((Duration timestamp) {
      final int inHours = minimumTime?.inHours ?? 0;
      final int inMinutes = minimumTime?.inMinutes ?? 0;
      final int inSelectedHour = selectedHour ?? 0;

      if (minimumTime != null &&
          inHours == inSelectedHour &&
          inMinutes.remainder(60) > selectedMinute.remainder(60)) {
        _animateColumnControllerToItem(minuteController, inMinutes);
      }
    });
  }
          ...
          return Semantics(
            ...
            child: _buildPickerNumberLabel(
                minute > 9
                    ...
                    : "0${localizations.timerPickerMinute(minute)}",
                additionalPadding),
          );
        }),
      ),
